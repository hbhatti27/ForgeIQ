...
export default function FrontPhotoUpload() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const storedUrl = localStorage.getItem('frontPhotoUrl');
  ...
  const handleFileChange = (e) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (data.url) {
          localStorage.setItem('frontPhotoUrl', data.url);
          setPreviewUrl(data.url);
          setUploadSuccess(true);
          console.log('✅ Front photo saved and preview set:', data.url);
        } else {
  ...
  const handleContinue = (e) => {
    e.preventDefault();
    if (!file) return alert('Please upload your front photo.');
    if (!uploadSuccess) return alert('Upload not completed yet!');
    router.push('/intake/side-photo');
  };

  return (
    <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center px-4 py-10">
      <h1 className="text-3xl font-bold text-orange-500 mb-6">Upload Front Physique Photo</h1>
